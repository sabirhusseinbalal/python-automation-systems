### python-bots-automation

This repository is part of a structured Python learning roadmap,
progressing from beginner to advanced real-world projects.

## 📌 Planned Projects

- Whatsapp Bot
- Send Emails
- Store Emails in CSV
- Fetch Tweets & Save CSV
- Language Translator
- Text to Speech
- Speech to Text
- XKCD Comics Downloader

### Clone the repository using the command:
   ```bash
   git clone https://github.com/Sabirhusseinbalal/python-bots-automation.git
   ```


## 🐍 Python Project Roadmap
1. [Python Basics – Mini Projects](https://github.com/sabirhusseinbalal/python-basics-mini-projects)
2. [File Handling & OS Automation](https://github.com/sabirhusseinbalal/python-file-system-automation)
3. [Web, Network & API Automation](https://github.com/sabirhusseinbalal/python-web-network-projects)
4. [Image, Video & Media Processing](https://github.com/sabirhusseinbalal/python-media-processing)
5. [Security, Encryption & Utilities](https://github.com/sabirhusseinbalal/python-cli-gui-apps)
6. [CLI, GUI & Small Apps](https://github.com/sabirhusseinbalal/python-security-utilities)
7. 👉 **Bots & Automation**
8. [Advanced Python Projects](https://github.com/sabirhusseinbalal/python-advanced-projects)
