...
The Telegram bot token is stored in a `.env` file instead of being written directly into the Python source code.
...
Before publishing the project, make sure the real bot token is removed from the repository and is never exposed in source code, screenshots, logs, or documentation.
